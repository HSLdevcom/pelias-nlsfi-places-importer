...
var createDocumentGenerator = function(kunnat, maakunnat) {
  return map_stream.obj(function(record) {
    ...
    var mmlDoc = new Document(record.type, 'venue', "mml-" + record["pnr:paikkaID"]);
    ...
    var maakuntaNimi = maakunnat[maakuntaKoodi][0].text;
    var kuntaNimi = kunnat[kuntaKoodi][0].text;
    var kunta = kunnat[kuntaKoodi];

    mmlDoc.setAdmin('locality', kuntaNimi);
    mmlDoc.setAdmin('admin1', maakuntaNimi);
    mmlDoc.setAdmin('admin0', 'Finland');
    mmlDoc.setAlpha3('FIN');


/*    for(var i in kunta) {
      mmlDoc.addParent('locality', kunta[i].text);
      } */

    try {
      mmlDoc.addParent('county', maakuntaNimi, maakuntaKoodi.toString());
    } catch(err) { logger.info('invalid county', err) }
    try {
      mmlDoc.addParent('country', 'Finland', "85633143", "FIN");
    } catch(err) { logger.info('invalid value', err);
    }

//  mmlDoc.addParent('country', 'Suomi');
...
//    logger.info("###DOC: ", JSON.stringify(mmlDoc));
    return mmlDoc;
  });
};
