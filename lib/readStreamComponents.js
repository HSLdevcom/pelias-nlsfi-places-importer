...
var createDocumentGenerator = function(kunnat, maakunnat) {
  return map_stream.obj(function(record) {
    ...
    mmlDoc.setAdmin('admin0', 'Finland');    mmlDoc.setAlpha3('FIN');

    for(var i in kunta) {
      try { mmlDoc.addParent('locality', kunta[i].text, kuntaKoodi.toString()); }
      catch (err) { logger.info('invalid locality', err) }
    }

    for(var i in maakunta) {
      try { mmlDoc.addParent('county', maakunta[i].text, maakuntaKoodi.toString()); }
